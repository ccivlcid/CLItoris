  ...
  return (
    <div className="space-y-4">
      <div className="border border-gray-700 bg-[#16213e] p-6 space-y-4">
        <p className="text-gray-600 text-xs font-mono">// channel subscriptions</p>
        ...
        <div className="space-y-2 opacity-40 pointer-events-none select-none">
          <p className="text-gray-500 font-mono text-xs">$ channel --list</p>
          <p className="text-gray-600 font-mono text-sm">&gt; no subscriptions yet.</p>
        </div>
      </div>

      <div className="border border-gray-700 bg-[#16213e] p-6 space-y-3">
        <p className="text-gray-600 text-xs font-mono">// trending channels (preview)</p>
        <div className="flex flex-wrap gap-2">
          {TRENDING.map((tag) => (
            <span
              key={tag}
              className="px-2 py-1 border border-gray-700 text-gray-600 font-mono text-xs"
            >
              {tag}
            </span>
          ))}
        </div>
        <p className="text-gray-700 font-mono text-xs">// join channels once Phase 3 ships</p>
      </div>
    </div>
  );
}
