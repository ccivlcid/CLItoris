...
**[terminal.social](https://terminal.social)** · **[English](./README.md)** · **[로드맵](./docs/PROGRESS.md)** · **[Discord](https://discord.gg/clitoris)**
...
|                |                                     |                                             |                                  |                                           |                                        |                                                |                                              |
| -------------- | ----------------------------------- | ------------------------------------------- | -------------------------------- | ----------------------------------------- | -------------------------------------- | ---------------------------------------------- | -------------------------------------------- |
| **연동** **서비스** | 🐙 **GitHub** OAuth · 스타 이슈 · PR 웹훅 | 🟣 **Anthropic** Claude Sonnet Claude Haiku | 🟢 **OpenAI** GPT-4o GPT-4o-mini | 🔵 **Google** Gemini 2.5 Pro Gemini Flash | 🦙 **Ollama** Llama · Mistral 모든 로컬 모델 | 🔀 **라우터** OpenRouter Together · Groq Cerebras | 🤖 **AI 에이전트** OpenClaw · Dify Coze · Custom |


*API가 있으면 연결됩니다. 로컬로 돌아가면 더 좋습니다. 에이전트면 채팅하세요.*
...
|                                                                                 |                                                                                  |                                                                                  |
| ------------------------------------------------------------------------------- | -------------------------------------------------------------------------------- | -------------------------------------------------------------------------------- |
| ⚡ 자동 포스팅 main에 push, PR 머지, 릴리스 배포 — CLItoris가 자동으로 피드에 포스팅합니다. 웹훅 한 번 설정하면 끝. | 📥 활동 임포트 한 번의 클릭으로 최근 GitHub 이벤트를 동기화하세요. Push, PR, 릴리스, 스타, 포크 — 전부 포스트가 됩니다. | 💬 목소리를 얹으세요 GitHub은 diff를 보여줍니다. 당신은 이야기를 추가하세요 — 뭘 배웠는지, 뭐가 답답했는지, 뭐가 자랑스러운지. |
...
|                                             |                                     |                                         |                                          |
| ------------------------------------------- | ----------------------------------- | --------------------------------------- | ---------------------------------------- |
| **🌱 기여** **그래프** 프로필에 잔디 히트맵               | **👥 팔로우** **동기화** GitHub 친구 자동 팔로우 | **📊 활동** **임포트** Push · PR · 릴리스 → 포스트 | **🔔 알림** GitHub 알림 앱 내에서                |
| **🪝 웹훅** **자동 포스팅** Push, 머지, 릴리스 → 즉시 포스트 | **⭐ 스타** GitHub 스타 레포 브라우징          | **📋 이슈 &** **PR 리뷰** 할당된 이슈와 리뷰 요청 추적  | **🔑 아이덴티티** GitHub = CLItoris 별도 계정 불필요 |
...
|                                                                        |                                                                                                            |                                                                                       |
| ---------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------- |
| 🖥️ 듀얼 포맷 포스트 모든 포스트가 원문과 CLI 표현을 나란히 보여줍니다. AI가 의도, 감정, 해시태그를 자동 추출. | 🤖 9+ AI 프로바이더 Claude, GPT-4o, Gemini, Ollama, OpenRouter, Together, Groq, Cerebras 및 모든 OpenAI 호환 엔드포인트. | 💬 AI 에이전트 챗 외부 AI 에이전트 (OpenClaw, Dify, Coze)를 연결하고 CLItoris 안에서 바로 대화하세요. 스트리밍 응답. |
| 📊 레포 분석 AI로 GitHub 레포 분석. 아키텍처 리포트, PPTX 슬라이드, 애니메이션 비디오 워크스루.       | 🔍 GitHub 탐색 트렌딩 레포, 저장소 및 사용자 검색 — 터미널 미학으로 구현된 GitHub 검색.                                               | 📱 모바일 퍼스트 터치에 최적화된 설계. 하단 네비, 전체 화면 알림, 프로필 탭, 글래스 UI.                              |
| ⌨️ 키보드 우선`j`/`k` 탐색, `s` 스타, `r` 답글, `/` 작성. 마우스 불필요.                 | 🌍 4개 언어 영어, 한국어, 중국어, 일본어 전체 UI. 어떤 언어로든 작성 가능 — AI가 나머지를 처리.                                            | ✉️ 다이렉트 메시지 아무 사용자에게 비공개 메시지. 실시간 대화 스레드. 모바일 최적화 인박스.                               |
...
당신의 코드는 이미 이야기를 하고 있습니다. 목소리를 주세요.

**>_CLI**toris  ·  terminal.social